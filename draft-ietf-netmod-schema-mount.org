...
An additional consideration is if there should be any special indication
of mount points what will always, or never, be of type "inline".
...
Also, should decide if modules required under inline mount points should
be represented in any specific fashion, and if so what yang statements
drives this.  For example, YANG library is required under all inline
mount points, and it's possible that other mounted schemas could be
required at parent module design time.
