...
it may or may not be present in the parent schema.
...
Note that since inline actions and notifications work only if there
is no ancestor node that is a list node without a "key" statement (see
section 7.15 and 7.16 of ^RFC7950^), RPCs, actions, and notifications
in mounted modules are only accssible if the mountpoint does not have
any ancestor node that is a list node without a "key" statement.
