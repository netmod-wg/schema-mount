...
Generally, the modules that are mounted under a mount point has no
relation to
...
data may have no relationship to the data contained in the parent
module.  Exceptions are possible, and such needs to be defined in the
model defining the exception, e.g., YANG library data as noted below
and the interface module in ^I-D.ietf-rtgwg-lne-model^. 
...
point. Note, that in this case a mount point may included a mounted YANG
...
- a list in the YANG library format specifying all YANG modules
  (augmentations, and revisions etc.) that are implemented or imported
  in the mounted schema;
...
contain modules with RPCs, actions, and notifications MUST NOT have any
